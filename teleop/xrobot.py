...
- Right controller key_one (A button): Cycle through idle -> teleop -> pause -> teleop...
- Right controller key_two (B button): Not used
- Left controller key_one (X button): Exit program from any state
- Left controller key_two (Y button): Toggle recording on/off (requires --record_pkl flag)
...
Recording:
- Enable with --record_pkl flag
- Press Y button (left controller key_two) to start/stop recording
- Only records during active recording state
- Saves to xrobot_retargeting_YYYYMMDD_HHMMSS.pkl on exit
...
from gmr.motion_retarget import GeneralMotionRetargeting as GMR
from gmr.robot_motion_viewer import draw_frame
from gmr.params import ROBOT_XML_DICT, ROBOT_BASE_DICT
from gmr.neck_retarget import human_head_to_robot_neck
from gmr.xrobot_utils import XRobotStreamer

from assets.params import DEFAULT_MIMIC_OBS, DEFAULT_HAND_POSE
from utils.rotations import euler_from_quaternion_np, quat_diff_np, quat_rotate_inverse_np
...
class StateMachine:
    def __init__(self, enable_smooth=False, smooth_window_size=5, use_pinch=False):
        ...
        self.left_key_two_was_pressed = False  # For Y button (recording toggle)
        # Recording state
    ...
    def update(self, controller_data):
        ...
        left_key_two_current = controller_data.get('LeftController', {}).get('key_two', False)  # Y button
        ...
        # Handle left key_two (Y button) - toggle recording
        if left_key_two_just_pressed:
            self.is_recording = not self.is_recording
            if self.is_recording:
                print("\n🔴 [RECORDING STARTED] - Press Y again to stop")
            else:
                print("\n⏹️  [RECORDING STOPPED]")
...
class XRobotTeleopToRobot:
    def __init__(self, args):
        ...
        #pkl recorder
        self.record_pkl = args.record_pkl
        print(f"Record mimic obs to pickle: {self.record_pkl}")
        self.pkl_recordings = [] if self.record_pkl else None
    ...
    def send_to_redis(self, mimic_obs, neck_data=None):
        ...
        if self._send_count % 100 == 0:
            ...
            if self.record_pkl:
                recording_status = "🔴 RECORDING" if self.state_machine.is_recording else "⚪ NOT RECORDING"
        ...
        # Record to pkl if enabled and recording is active
        if self.record_pkl and self.state_machine.is_recording and mimic_obs is not None:
            self.record_retargeting_data(mimic_obs, neck_data)

    
    def record_retargeting_data(self, mimic_obs, neck_data):
        """Record retargeting data to pkl format (matching g1_sim.py format)"""
        if not self.record_pkl or self.last_qpos is None:
            return
            
        # Extract data from last_qpos (which contains the full robot state)
        # last_qpos format: [pos(3), quat(4), joints(29)]
        root_pos = self.last_qpos[0:3]  # ✅ Extract root position
        quat = self.last_qpos[3:7]
        
        # Convert quaternion to roll-pitch-yaw
        rpy = R.from_quat(quat[[1, 2, 3, 0]]).as_euler('xyz')  # scipy expects [x,y,z,w]
        
        dof_pos = self.last_qpos[7:]  # ✅ 直接从 last_qpos 提取关节数据（29维）

        
        # Record in the same format as g1_sim.py
        # Use retargeted_dof_pos for both dof_pos and target_dof_pos
        proprio_data = {
            'timestamp': time.time(),
            'root_pos': root_pos.tolist(),  # ✅ Added root position
            'dof_pos': dof_pos.tolist(),  # GMR retargeted joint positions
            'dof_vel': np.zeros(29).tolist(),  # We don't have velocity in retargeting
            'rpy': rpy.tolist(),
            'ang_vel': np.zeros(3).tolist(),  # We don't have angular velocity
            'target_dof_pos': dof_pos.tolist(),  # Same as dof_pos for visualization
        }
        
        if neck_data is not None:
            proprio_data['neck_data'] = neck_data
            
        self.pkl_recordings.append(proprio_data)
    ...
    def save_pkl_recordings(self):
        """Save recorded pkl data to file in g1_motion.py and RobotMotionViewer compatible format"""
        if not self.pkl_recordings:
        ...
        filename = os.path.join(recordings_dir, f"xrobot_retargeting_{timestamp}.pkl")
        
        try:
            n_frames = len(self.pkl_recordings)
            
            # G1机器人的关键身体链接（用于 RobotMotionViewer）
            g1_body_links = [
                'pelvis',           # 0: 骨盆（根）
                'torso_link',       # 1: 躯干
            ...
            # Extract data arrays for RobotMotionViewer format
            root_pos_list = []
            root_rot_list = []  # quaternion [x, y, z, w]
            dof_pos_list = []
            
            for frame in self.pkl_recordings:
                # Extract root position
                root_pos = np.array(frame['root_pos'])
                
                # Convert rpy to quaternion [x, y, z, w]
                rpy = np.array(frame['rpy'])
                root_quat_xyzw = R.from_euler('xyz', rpy).as_quat()  # [x,y,z,w]
                
                root_pos_list.append(root_pos)
                root_rot_list.append(root_quat_xyzw)
                dof_pos_list.append(np.array(frame['dof_pos']))
            
            # Convert to numpy arrays
            root_pos_array = np.array(root_pos_list)      # (n_frames, 3)
            root_rot_array = np.array(root_rot_list)      # (n_frames, 4) xyzw
            dof_pos_array = np.array(dof_pos_list)        # (n_frames, 29)
            
            # ✅ 自动生成 local_body_pos
            num_bodies = len(g1_body_links)
            local_body_pos = np.zeros((n_frames, num_bodies, 3))
            ...
            # ✅ 创建完整的motion数据（包含所有必需字段）
            motion_data = {
                'fps': self.target_fps,
                'root_pos': root_pos_array,
                'root_rot': root_rot_array,
                'dof_pos': dof_pos_array,
                'local_body_pos': local_body_pos,     # ✅ 自动填充
                'link_body_list': g1_body_links        # ✅ 自动填充
            }
            ...
            print(f"\n✅ Saved {n_frames} frames to {filename}")
            print(f"   Total recording duration: {n_frames / self.target_fps:.2f} seconds")
            print(f"   Format: RobotMotionViewer compatible (auto-fixed)")
            print(f"   Saved to: {os.path.abspath(filename)}")
            print(f"   Body links: {len(g1_body_links)} parts")
            print(f"   local_body_pos shape: {local_body_pos.shape}")
            
        except Exception as e:
            print(f"\n❌ Error saving pkl file: {e}")
            import traceback
            traceback.print_exc()
    ...
    def run(self):
        ...
        with mjv.launch_passive(
            ...
            try:
                ...
                # Save pkl recordings if enabled
                if self.record_pkl and self.pkl_recordings:
                    self.save_pkl_recordings()
...
def parse_arguments():
    ...
    parser.add_argument(
        "--record_pkl",
        action="store_true",
        help="Whether to record mimic observations and neck data to a pickle file for offline analysis.",
    )
