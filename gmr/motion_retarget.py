...
class GeneralMotionRetargeting:
    ...
    def constrain_foot_rotation(self, human_data):
        """约束脚部旋转，使其更贴近地面"""
        for body_name in human_data.keys():
            ...
            # 限制 pitch 和 roll 角度（保持脚掌接近水平）
            euler[0] = np.clip(euler[0], -0.3, 0.3)  # roll: ±17 度
            euler[1] = np.clip(euler[1], -0.5, 0.5)  # pitch: ±29 度
            
            # 重新构建四元数
            new_rot = R.from_euler('xyz', euler)
            human_data[body_name][1] = new_rot.as_quat(scalar_first=True)
    ...
    def update_targets(self, human_data, offset_to_ground=False):
        ...
        human_data = self.constrain_foot_rotation(human_data)
        if offset_to_ground:
